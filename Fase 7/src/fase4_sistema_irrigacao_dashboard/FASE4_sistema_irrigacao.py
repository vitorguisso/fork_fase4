...
import cx_Oracle
from fase5_analise_aws.aws_alertas import enviar_alerta_aws  # integração com AWS SNS
...
def ler_tudo():
    ...
    for r in registros:
        print(
            f"id: {r[0]} | Umidade: {r[1]:.2f}% | pH: {r[2]:.2f} | "
            f"Fósforo: {r[3]} | Potássio: {r[4]} | Bomba: {r[5]}"
        )
    print("-----------------------------")
...
def criar_registro():
    ...
    # Enviar alerta AWS com base no novo registro
    print("\nEnviando alerta AWS baseado nos novos dados do sensor (novo registro)...")
...
def atualizar():
    ...
    if campo == "umidade":
        while True:
            try:
                valor = float(input("Novo valor de umidade (0 a 100): "))
                if 0 <= valor <= 100:
                    break
        ...
        cursor.execute(
            "UPDATE sistema_irrigacao SET umidade = :1 WHERE id = :2",
            (valor, id_registro)
        )
        # recalcula estado da bomba
        bomba = calcular_bomba(valor)
        cursor.execute(
            "UPDATE sistema_irrigacao SET bomba = :1 WHERE id = :2",
            (bomba, id_registro)
        )
        ...
        cursor.execute(
            "UPDATE sistema_irrigacao SET ph = :1 WHERE id = :2",
            (valor, id_registro)
        )

    else:  # fosforo ou potassio
        while True:
        ...
        cursor.execute(
            f"UPDATE sistema_irrigacao SET {campo} = :1 WHERE id = :2",
            (valor, id_registro)
        )
    ...
    # Buscar os valores atuais do registro para mandar no alerta
    cursor.execute(
        "SELECT umidade, ph, fosforo, potassio FROM sistema_irrigacao WHERE id = :1",
        (id_registro,)
    )
    linha = cursor.fetchone()
    if linha:
        umidade_atual, ph_atual, fosforo_atual, potassio_atual = linha
        print("\nEnviando alerta AWS baseado nos novos dados do sensor...")
        enviar_alerta_aws(umidade_atual, ph_atual, fosforo_atual, potassio_atual)
    else:
        print("Não foi possível encontrar o registro para envio de alerta.")
